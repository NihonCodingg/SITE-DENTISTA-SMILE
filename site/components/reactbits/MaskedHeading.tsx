...
/**
 * Headline preenchida por uma imagem através do recorte das próprias letras —
 * Task 20, a headline do hero. Origem:
 * src/ts-tailwind/TextAnimations/MaskedHeading/MaskedHeading.tsx (ver
 ...
 *    fonte única é `useCapability()`. Sob redução: sem reveal, sem deriva,
 *    sem parallax; o preenchimento fica, estático.
 * 3. **`aria-hidden` no conteúdo.** O original espalha as palavras em spans
 *    SEM espaço real entre elas (o espaço é `content` de CSS) — um leitor de
 *    tela leria "SEUNOVOSORRISO…". O conteúdo visual inteiro fica escondido
 *    e o nome acessível vai no heading que envolve o componente
 *    (`tituloAriaLabel` do SectionHeading), como o SplitText já fazia.
 * 4. **O laço de `requestAnimationFrame` pausa** fora da viewport, com a aba
 *    oculta e sob `reducedMotion` — o original roda para sempre.
 * 5. **Modo `video` removido** (o site não hospeda vídeo) e **`<img>`
 *    trocado por `next/image`**, como nos demais vendorizados.
 * 6. **Escrita de ref movida do render para `useLayoutEffect`** (regra
 *    `react-hooks/refs` deste Next).
 */
...
const MaskedHeading: React.FC<MaskedHeadingProps> = ({
  ...
  useEffect(() => {
    ...
    let raf = 0;
    let last = performance.now(); // reatribuída em ligar()
    let clock = 0;
    ...
    // O original roda o rAF para sempre. Aqui ele só roda com a seção
    // visível, a aba em primeiro plano e sem reducedMotion — deriva e
    // parallax são decorativos; o preenchimento estático não precisa de
    // quadro nenhum.
    let visivel = true;
    const ligar = () => {
      if (raf) return;
    ...
    const avaliar = () => {
      if (!reducedMotion && visivel && !document.hidden) ligar();
      else desligar();
    };
  ...
  }, [reveal, trigger, duration, stagger, words, reducedMotion]);
  ...
  return (
    <TagAny
      ...
      {...rest}
    >
      <span ref={measureRef} aria-hidden="true" className="text-transparent">
        {words.map((word, i) => (
          <span
            key={`${word}-${i}`}
      ...
      <span ref={revealRef} className="absolute inset-0 block pointer-events-none">
        <span className="absolute inset-0 block" style={{ clipPath: `url(#${clipId})` }}>
          <span ref={mediaRef} className="absolute inset-0 block [will-change:transform,filter]">
            <Image className="select-none object-cover" src={src} alt="" fill sizes="100vw" draggable={false} />
          </span>
        </span>
      </span>
    </TagAny>
