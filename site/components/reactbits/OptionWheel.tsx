...
/**
 * Roda de opções — Task 20 ("Soluções que transformam sorrisos").
 * Origem: src/ts-tailwind/Components/OptionWheel/OptionWheel.tsx
 ...
 * 2. **Rótulo acessível por prop.** O original cravava `aria-label="Option
 *    wheel"`, em inglês, num site em português.
 * 3. **`aria-activedescendant`.** O `role="listbox"` do original não dizia
 *    qual opção está ativa — quem usa leitor de tela ouvia a lista mas não
 *    a mudança de seleção ao navegar pelas setas. Cada `role="option"`
 *    ganhou `id`, e o listbox aponta para o ativo.
 * 4. **Som desligado.** O original aceita `soundUrl` e toca um clique a cada
 *    passo. O default já era vazio; aqui a prop foi REMOVIDA junto com o
...
export interface OptionWheelProps {
  ...
  loop?: boolean;
  draggable?: boolean;
  /** Nome acessível do listbox. O original cravava "Option wheel". */
  rotulo?: string;
  className?: string;
}
...
const OptionWheel = ({
  ...
  // O original escrevia estes dois refs DURANTE o render, o que a regra
  // `react-hooks/refs` do eslint do Next 16 reprova (mesma correção que o
  // `DepthCarousel` levou). `useLayoutEffect` roda antes da pintura, então o
  // loop de animação continua lendo a configuração atual no mesmo quadro.
  useLayoutEffect(() => {
  ...
  const runFrame = useCallback(function quadro(now: number) {
    ...
    // Expressão de função NOMEADA (`quadro`): o original se referenciava
    // por `runFrame`, a const que ainda não existe neste ponto — uso antes
    // da declaração, reprovado pelo eslint deste Next. Um nome próprio
    // resolve sem ref nenhum, e o agendamento continua idêntico.
    rafRef.current = settled ? null : requestAnimationFrame(quadro);
  }, []);
