...
// Só 'height' e 'width' precisam de variante responsiva nesta base de código
// (nenhum consumidor usa strength/opacity responsivos) — o original aceitava
// `keyof GradualBlurProps` genérico e montava a chave "mobile"+Cap(key) via
// concatenação de string, o que só tipava com `any`. Uma tabela fechada
// housing as três chaves por eixo elimina o `any` sem tirar a funcionalidade
// que este projeto usa.
type ResponsiveKey = 'height' | 'width';
const RESPONSIVE_FIELDS: Record<
  ResponsiveKey,
  { mobile: keyof GradualBlurProps; tablet: keyof GradualBlurProps; desktop: keyof GradualBlurProps }
> = {
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const fields = RESPONSIVE_FIELDS[key];
    const calc = () => {
      const w = window.innerWidth;
      let v = config[key];
      if (w <= 480 && config[fields.mobile]) v = config[fields.mobile] as string;
      else if (w <= 768 && config[fields.tablet]) v = config[fields.tablet] as string;
      else if (w <= 1024 && config[fields.desktop]) v = config[fields.desktop] as string;
      setVal(v);
    };
